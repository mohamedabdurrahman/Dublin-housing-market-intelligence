...
Use `data/dublin_housing_summary.csv` as the raw fact input and the output of `transform_housing_data.sql` as the annual analytical table. Relate the calendar table to `calendar_year`; retain `local_authority` as the primary geography. The core measures are completions, planning permissions, completion YoY growth, delivery share, and the permissions-to-completions lead-time proxy.

## Page 1: Executive KPI Overview

Purpose: give senior decision-makers a fast read on supply delivery and forward pipeline.

- KPI cards: latest completions, latest planning permissions, completion YoY growth, and four-authority delivery share.
- Line chart: quarterly completions versus planning permissions, with a slicer for authority.
